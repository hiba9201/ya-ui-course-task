function createQuest(quest) {
    ...
    questImageLink.setAttribute('href', `/scene/${quest.startScene}`);

    const image = document.createElement('img');
    image.classList.add('quest__image');
    image.setAttribute('alt', quest.name);
    image.setAttribute('src', quest.image || `${state.static}default-quest-image.png`);
    ...
    title.setAttribute('href', `/scene/${quest.startScene}`);
...
function createTag(tag) {
    ...
    tagLink.setAttribute('href', `/tags/${tag.engName}`);
