...
function Quests(props: { fetchLink: string; tag?: string }) {
    ...
    const [ quests, setQuests ] = useState(new Array<QuestProps>());
    ...
    useEffect(() => {
        ...
        fetch(`${fetchLink}?limit=${state.limit}&offset=${state.offset}`)
            ...
            .then(questsData => {
                setQuests([...quests, ...questsData]);

                if (questsData.length < state.limit) {
                    setState({ ...state, hasMore: false });
                } else {
                    setState({ ...state, offset: state.offset + state.limit });
                }
            }, () => setQuests(new Array<QuestProps>()))
